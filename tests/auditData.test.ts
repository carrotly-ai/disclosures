...
const filing = (id: string, date: string) => ({
  type: "filing",
  id,
  attributes: {
    fxo_id: `${lei}-${id}`,
...
test.each([null, undefined, "", " ", true, false, "NaN", "0x10"])(
  "ESEF rejects missing/nonnumeric values: %p",
  async (value) => {
    const facts = await getEsefFinancials(lei, ["revenue"], {
      fetchFn: async (url) =>
        Response.json(
          url.includes("/api/filings")
            ? { data: [filing("a", "2026-01-01")] }
...
test("ESEF preserves genuine zero and per-share units", async () => {
  const facts = await getEsefFinancials(lei, ["revenue", "eps_basic"], {
    fetchFn: async (url) =>
      Response.json(
        url.includes("/api/filings")
          ? { data: [filing("a", "2026-01-01")] }
  ...
  expect(facts.map((f) => [f.concept, f.value, f.unit])).toEqual([
    ["revenue", 0, "GBP"],
    ["eps_basic", 2.5, "GBP/shares"],
  ]);
});

test.each([false, true])(
  "ESEF amendments win within the same day regardless of list order: %p",
  async (reverse) => {
    const data = [
      filing("old", "2026-01-01T08:00:00Z"),
      filing("new", "2026-01-01T09:00:00Z"),
    ];
        ...
        Response.json(
          url.includes("/api/filings")
            ...
            : {
                facts: {
                  revenue: {
                    value: url.endsWith("/old.json") ? "100" : "200",
                    dimensions: dimensions(),
                  },
                },
              },
...
test.each(COMPANY_JURISDICTIONS.filter((j) => j !== "US"))(
  "PrivateRaises rejects %s without network access",
  async (jurisdiction) => {
    let calls = 0;
...
test.each([403, 429, 500])(
  "PrivateRaises reports document HTTP %s as failure, not absence",
  async (status) => {
    const tool = createTools({
      env: { DISCLOSURES_USER_AGENT: "Audit audit@example.com" },
      fetchFn: async (url) =>

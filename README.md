...
- **Official sources only** — SEC EDGAR, GLEIF, UK Companies House, FCA NSM, filings.xbrl.org, Korea DART, Japan EDINET, China cninfo, India BSE, Taiwan TWSE, Brazil CVM, Germany BaFin, France info-financiere OAM + recherche-entreprises, Hong Kong HKEXnews, Singapore ACRA, Thailand DBD, Indonesia IDX, Dubai DFM.
...
| [DFM](https://www.dfm.ae/) (Dubai Financial Market) | `AE` | None | **Dubai only, not the whole UAE.** Resolution (DFM + Nasdaq Dubai securities by symbol or English/**Arabic** name, LEI via GLEIF), per-issuer efsah (`إفصاح`) disclosures with a direct keyless PDF per attachment, and `CompanyDocument` by the efsah `r_path` (metadata / extracted text / download). ADX Abu Dhabi (`403`), the DIFC public register (`429`) and ADGM registration (`403`) are bot-walled from a server, so the UAE's largest issuers (ADNOC group, IHC, Aldar) are **out of scope** — every AE response says so. Owners/insiders/financials arrive as free-text PDFs with no XBRL or SCA threshold register — honest unsupported. Exchange ©: link-first, on-demand, no bulk redistribution. |

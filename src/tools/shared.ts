  ...
  company: z
    ...
    .describe(
      "Company name or jurisdiction-specific identifier: ticker/CIK (US), " +
        ...
        "6-digit stock code (KR), or 20-character LEI",
    ),
  jurisdiction: z
    .enum(["US", "GB", "KR", "JP"])
    .optional()
    .describe(
      "Jurisdiction to search: US (SEC EDGAR), GB (Companies House), or " +
        "KR (OpenDART/DART). JP (EDINET) is planned. Omit for the existing US default.",
    ),
};
...
export function failureResult(company: string, error: unknown): ToolResult {
  ...
  if (
    /not found|no sec company found|no gleif entity|no companies house company found|no opendart company found/i
      .test(message)
  ) {

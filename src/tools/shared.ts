  ...
  company: z
    ...
    .describe(
      "Company name or jurisdiction-specific identifier: ticker/CIK (US), " +
        ...
        "4-letter IDX ticker / kode emiten (ID), DFM issuer symbol (AE), or " +
        "20-character LEI",
    ),
  jurisdiction: z
    .enum([
...
export function failureResult(company: string, error: unknown): ToolResult {
  ...
  if (
    /not found|no sec company found|no gleif entity|no companies house company found|no opendart company found|no edinet company found|no cninfo company found|no bse company found|no twse company found|no cvm company found|no bafin company found|no hkexnews company found|no acra company found|no afm issuer found|no idx company found|no dfm company found/i
      .test(message)
  ) {
